...
(function (global, name, factory) {
	...
	// return a dom element (path object) that can be attached to an existing <g> object
	toPaths: function(g) {
	    for (var color in this._pathsByColor) {
		// var path = document.createElement('path');
		var path = document.createElementNS("http://www.w3.org/2000/svg", "path")
		path.setAttribute('fill', color);
		path.setAttribute('d', this._pathsByColor[color].dataString);
		g.appendChild(path)
	    }
	}
    ...
    function update(el, hash, padding) {
	    ...
	    isG = el["tagName"].toLowerCase() == "g",
            isCanvas = el["tagName"].toLowerCase() == "canvas";
        
        // Ensure we have a supported element
        if (!isSvg && !isG && !(isCanvas && "getContext" in el)) {
            return;
        }
        
        var width = Number(el.getAttribute("width")) || Number(el.attributes["data-width"].value) || el.clientWidth || 0,
            height = Number(el.getAttribute("height")) || Number(el.attributes["data-height"].value) || el.clientHeight || 0,
            renderer = (isSvg || isG) ? new SvgRenderer(width, height) : new CanvasRenderer(el.getContext("2d"), width, height),
            size = Math.min(width, height);
        
        // Draw icon
        iconGenerator(renderer, hash, (isG ? -size/2 : 0), (isG ? -size/2 : 0), size, padding, getCurrentConfig());
